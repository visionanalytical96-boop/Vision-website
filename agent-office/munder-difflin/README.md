...
## Checking the manifests still import cleanly

`validate-hires.mjs` runs our manifests through Munder Difflin's own
`validateHireManifest()`, so a pass means the app will accept the file - not that it
matches a schema we copied by hand. Re-run it after editing any manifest, and after
upgrading the app:

```bash
git clone --depth 1 https://github.com/chaitanyagiri/munder-difflin /tmp/munder-difflin
MD_REPO=/tmp/munder-difflin node --experimental-strip-types validate-hires.mjs
```

It exits non-zero if any manifest would be rejected.
