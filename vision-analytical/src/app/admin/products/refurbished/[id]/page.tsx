...
export default async function EditRefurbishedPage(props: PageProps<'/admin/products/refurbished/[id]'>) {
  ...
  const [instrument, categories] = await Promise.all([getAdminRefurbishedById(id), getAllCategories()]);
  if (!instrument) notFound();

  return <RefurbishedForm categories={categories} instrument={instrument} />;
}
