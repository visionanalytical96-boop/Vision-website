  ...
  return (
    <form action={formAction} className="grid max-w-3xl gap-4 sm:grid-cols-2">
      <FormField label="Brand" htmlFor="brandId" error={state.errors?.brandId} required>
        <Select id="brandId" name="brandId" defaultValue={submittedOr(state.values, 'brandId', model?.brandId ?? '')} required>
      ...
      <FormField label="Technique" htmlFor="categoryId" error={state.errors?.categoryId}>
        <Select id="categoryId" name="categoryId" defaultValue={submittedOr(state.values, 'categoryId', model?.categoryId ?? '')}>
      ...
      <FormField label="Model name" htmlFor="name" error={state.errors?.name} required>
        <Input id="name" name="name" defaultValue={submittedOr(state.values, 'name', model?.name)} placeholder="e.g. Alliance e2695" required />
      </FormField>

      <FormField label="Slug" htmlFor="slug" error={state.errors?.slug} hint="Unique per brand." required>
        <Input id="slug" name="slug" defaultValue={submittedOr(state.values, 'slug', model?.slug)} placeholder="alliance-e2695" required />
      </FormField>

      <FormField label="Description" htmlFor="description" error={state.errors?.description} className="sm:col-span-2">
        <Textarea id="description" name="description" rows={2} defaultValue={submittedOr(state.values, 'description', model?.description ?? '')} />
      </FormField>

      <FormField label="Sort order" htmlFor="sortOrder" error={state.errors?.sortOrder} hint="Lower numbers appear first.">
        <Input id="sortOrder" name="sortOrder" type="number" defaultValue={submittedOr(state.values, 'sortOrder', model?.sortOrder ?? 0)} />
      </FormField>
      ...
      <label className="flex items-center gap-2 text-sm text-foreground sm:col-span-2">
        <input
          ...
          name="isPublished"
          value="true"
          defaultChecked={submittedChecked(state.values, 'isPublished', model?.isPublished ?? true)}
